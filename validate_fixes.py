...
def test_output_parsing():
    ...
    print(f"✓ Parsed {len(outputs_parsed)} enabled outputs:")
    ...
    if "USB" in outputs_parsed:
        print("✗ FAIL: USB should not be parsed (enable: false)")
        return False
    else:
        print("✓ USB correctly excluded (enable: false)")
...
def test_media_info_extraction():
    ...
    print(f"✓ Album: {album}")
    print(f"✓ Album Art: {image_url[:50]}..." if image_url else "✓ Album Art: None")
    ...
    if title == "Pink Pony Club" and artist == "Chappell Roan":
        ...
        print("✗ Failed to extract correct media information")
        return False
...
def test_volume_extraction():
    ...
    print(f"✓ Current Volume: {current_volume}/{max_volume}")
    print(f"✓ Volume (0-100 scale): {volume_percent}%")
    print(f"✓ Muted: {is_mute}")

    # In the real response: currenttVolume=200, maxVolume=200 → 100%
    if volume_percent == 100 and is_mute is False:
        print("✓ Volume extraction correct")
        return True
    else:
